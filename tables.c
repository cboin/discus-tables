...
void *insert_entry(unsigned int table_id)
{
    ...
    if (entry != NULL) {
	/*
	 * Ensure that 0 is never passed
	 * as a size arguement to malloc()
	 */
	if (td->size_of_entry == 0)
	    return NULL;
...
int delete(unsigned int table_id, func_tst is_func_tst)
{
    ...
    while (current != NULL && is_func_tst(current->value)) {
	next = current->next;
	free(current->value);
	free(current);
	/*
    ...
    while (current != NULL) {
	if (next != NULL && is_func_tst(next->value)) {
...
void create_table(unsigned int table_id, unsigned int entry_size,
